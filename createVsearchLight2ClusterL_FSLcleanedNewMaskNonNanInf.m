function createVsearchLight2ClusterL_FSLcleanedNewMaskNonNanInf(sb,areaM,dirT,dateS,nvoxelL)
...
nameNanMask = [data_path,'masks/NativeNotNanMask/',dirT,'/nonInfNonNan_Sub',num2str(vnpar(sb)),'.nii'];
...
[L,~] = defineSearchlight_volume_forParal(vnpar(sb),subDir,ROI,nanMask,'sphere',[(nvoxelL*70/100) nvoxelL]);
cd(based)
disp(pwd)
%L           = rsa.defineSearchlight_volume(ROI,nanMask,'sphere',[70 200]);%[70 216]);%
% L           = rsa.defineSearchlight_volume(ROI,nanMask);

nameDir = [data_path,'/NativeSearchLightDefinitions/',areaM,'/',dirT,'_newMask_nonInf/s',num2str(vnpar(sb))];
mkdir(nameDir);
%save([nameDir,'Ldefault.mat'],'L');
save([nameDir,'/L',num2str(nvoxelL),'_FSLcleaned',dateS,'.mat'],'L');
